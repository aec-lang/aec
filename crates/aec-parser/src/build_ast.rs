...
fn build_ui_block(pair: Pair<Rule>) -> Result<Vec<UiStatement>, ParseError> {
    let mut statements = Vec::new();
    for stmt_pair in pair.into_inner() {
        if let Some(stmt) = build_ui_statement(stmt_pair)? {
...
fn build_element_expr(pair: Pair<Rule>) -> Result<ElementExpr, ParseError> {
    let span = pair_span(&pair);
    let mut inner = pair.into_inner();
    ...
    for p in inner {
        match p.as_rule() {
            ...
            Rule::binding => {
                modifiers.push(ElementModifier::Binding(build_binding(p)?));
            }
            Rule::event_handler => {
...
fn build_binding(pair: Pair<Rule>) -> Result<Binding, ParseError> {
    let span = pair_span(&pair);
    let mut inner = pair.into_inner();
...
fn build_event_handler(pair: Pair<Rule>) -> Result<EventHandler, ParseError> {
    ...
    for op in inner {
        match op.as_rule() {
            Rule::member_op => {
                let prop = build_identifier(op.into_inner().next().unwrap())?;
            ...
            Rule::call_op => {
                ...
                expr = Expr::Call(Box::new(aec_ast::CallExpr {
                    callee: expr,
                    args,
                    span,
                }));
...
fn build_expr_from_identifier(pair: Pair<Rule>) -> Result<Expr, ParseError> {
    ...
    Err(build_error(pair_span(&pair), "expected identifier"))
}

fn build_ui_if(pair: Pair<Rule>) -> Result<UiIf, ParseError> {
    let span = pair_span(&pair);
    let mut inner = pair.into_inner();
...
fn build_ui_for(pair: Pair<Rule>) -> Result<UiFor, ParseError> {
    let span = pair_span(&pair);
    let mut inner = pair.into_inner();
